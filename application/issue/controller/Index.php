...
class Index extends Controller
{
	public function log_status(){
		...
		{
			echo "<script>location.href='".url('issue/index/login_page')."';</script>";exit;
		}
	}
	...
	public function insert_user_info(){
		...
		$data['sorce'] = input('sorce');
		$data['date'] = time();
		$result = db('issue_user') -> insertGetId($data);
		if($result){
		...
		switch($result['status']){
			...
			default:
				...
				if($money<=0||$num<=0){
					...
					$pack['residual_amount'] = $data['money']*100;
					$pack['residual_num'] = $data['num'];
					db('issue_red_pack') -> where('id',1) -> update($pack);
